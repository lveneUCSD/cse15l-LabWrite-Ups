...
<br> In Lab Report 3, I researched certain options that were useful for the ```find``` command in the terminal. In this lab, I will be doing something similar with the ```grep``` command. The general syntax of the ```grep``` command is 
```
grep [option] "pattern" <path>
```
where ```[option]``` represents the command-like option that induces a particular function of ```grep``` on the given ```"pattern"``` and where ```<path>``` is the location that is searched. The first command-line option helps the user find the first ```NUM``` occurrences of a particular ```"pattern"```. To do this, we use the option ```-m NUM "pattern"```, where ```NUM``` references the number of occurrences found before which ```bash``` will stop running and ```"pattern"``` references the pattern to be looked for. Consider the terminal input
```
